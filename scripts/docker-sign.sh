...
TIZEN_PROFILE_NAME="${TIZEN_PROFILE_NAME:-VertelesProfile}"
TIZEN_CERTIFICATE_PASSWORD="${TIZEN_CERTIFICATE_PASSWORD:-vertelesdevpwd}"
...
if [ ! -f "${AUTHOR_P12}" ]; then
    ...
    tizen certificate -a VertelesDev -p "${TIZEN_CERTIFICATE_PASSWORD}" -c CL -s Santiago -ct Santiago -o Verteles -u Dev -n Nico -e nico@example.com -f author
...
echo "⚡ Signing verteles.wgt..."
tizen package -t wgt -s "${TIZEN_PROFILE_NAME}" -- /workspace/verteles.wgt

echo "✅ App packaged and signed successfully! File is located at: /workspace/verteles.wgt"
